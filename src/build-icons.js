...
module.exports = ({
    ...
    return processIcons(icons, outputDir, {
        isTypeScriptOutput: typescript,
        monochrome,
        namedExport
    });
};
...
function createIndexFile(componentNames, outputDir, options) {
    const code = componentNames.length
        ? [
              ...
              "console.warn('You are using a whole package of yui-icon, ' + 'please use https://www.npmjs.com/package/babel-plugin-import to reduce app bundle size.')",
              "}",
              "\n\n",
              "/* eslint-disable */",
              "/* tslint:disable */",
              componentNames
                  .map(
                      name =>
                          `export {${
                              options.namedExport ? "" : "default as "
                          }${name}} from './${componentsDirName}/${name}';`
                  )
                  .join("\n"),
              "/* tslint:enable */",
              "/* eslint-enable */",
